...
export default function DashboardPage() {
  ...
  const totalAdSpend = metrics.reduce(
    (s, m) => s + (Number(m.ads_spend) || 0),
    0
  );
  const platformFees = metrics.reduce(
    (s, m) => s + (Number(m.platform_fees) || 0),
    0
  );
  const commissionFees = metrics.reduce(
    (s, m) => s + (Number(m.commission_fees) || 0),
    0
  );
  const logisticsFees = metrics.reduce(
    (s, m) => s + (Number(m.logistics_fees) || 0),
    0
  );
  const returnsAmount = metrics.reduce(
    (s, m) => s + (Number(m.returns_amount) || 0),
    0
  );
                ...
                <>
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <p className="text-red-500">
                      Returned: ₹{returnsAmount.toLocaleString()}
                    </p>
                    <p className="font-bold">
                      Net: ₹{(totalSales - returnsAmount).toLocaleString()}
                    </p>
                  </div>
                </>
              )}
              ...
              {loading ? (
                ...
                <>
                  ...
                  <p className="font-bold text-red-500">
                    Ad Spend: ₹{totalAdSpend.toLocaleString()}
                  </p>
                </>
              )}
            </CardContent>
